...
public class Snake extends ActiveObject {
  ...
  public boolean move() {
    ...
    if (direction == Direction.NONE &&
        snakeGame.getGameState() == PA8Constants.GAME_RUNNING) {
    ...
    if(grid.getCellNeighbor(currCell, direction).hasFruitLoop()) {
      ...
      FruitLoop fruitLoop = new
        FruitLoop(PA8Constants.FRUIT_LOOP_COLORS[colorIdx],
                  PA8Constants.BACKGROUND_COLOR, grid.getRandomEmptyCell(),
                  canvas); 
      }      
    }
    if (!gameWon)  {
   ...
   */
  public void die() {
    snakeGame.gameState(PA8Constants.GAME_OVER);
  }
  ...
  public void run() {
    while(!reset) {
      ...
      if (snakeGame.getGameState() == PA8Constants.GAME_RUNNING) {
        if(!move()) {
          reset = true;
          die();          
        }
